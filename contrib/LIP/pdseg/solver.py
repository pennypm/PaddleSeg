...
class Solver(object):
    ...
    def cosine_decay(self):
        ...
        def cosine_d(base_lr, step_each_epoch, epochs):
            ...
            with default_main_program()._lr_schedule_guard():
                ...
                with fluid.layers.control_flow.Switch() as switch:
                    with switch.case(cur_epoch < warmup_epoch):
                        decayed_lr = eta_min + cur_epoch*(base_lr - eta_min)/warmup_epoch
                        tensor.assign(decayed_lr, lr)
                    with switch.case(cur_epoch < start_cyc):
                        decayed_lr = eta_min + (base_lr-eta_min)*(1+ops.cos(math.pi*(cur_epoch-warmup_epoch)/(start_cyc-warmup_epoch)))/2
                        tensor.assign(decayed_lr, lr)
                    with switch.default():
                        epoch_per_cyc = fluid.layers.cast(fluid.layers.cast(cur_epoch - start_cyc,'int32') % cyc_epoch,'float32')
                        decayed_lr = eta_min + (cyc_base_lr-eta_min)*(1+ops.cos(math.pi*epoch_per_cyc/cyc_epoch)) / 2
                        tensor.assign(decayed_lr, lr)
            return lr 

        return cosine_d(cfg.SOLVER.LR, self.step_per_epoch, cfg.SOLVER.NUM_EPOCHS)
