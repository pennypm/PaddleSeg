...
def build_model(main_prog, start_prog, phase=ModelPhase.TRAIN):
    ...
    with fluid.program_guard(main_prog, start_prog):
        with fluid.unique_name.guard():
            ...
            label_edge = fluid.layers.data(
                name='label_edge', shape=grt_shape, dtype='int32')
            ...
            if ModelPhase.is_train(phase) or ModelPhase.is_eval(phase):
                py_reader = fluid.io.PyReader(
                    feed_list=[image, label,label_edge,  mask],
            ...
            [seg1, seg2], edge = model_func(image, class_num)
            
            if ModelPhase.is_train(phase) or ModelPhase.is_eval(phase):
                avg_loss = multi_softmax_with_loss([[seg1, seg2], edge], [label, label_edge],
                                                    mask, class_num)
            logits = seg2
            ...
            if ModelPhase.is_train(phase):
                ...
                return py_reader, avg_loss, decayed_lr, pred, [[seg1, seg2], edge], [label, label_edge], mask
