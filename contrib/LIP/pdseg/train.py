...
from vis_ace2p import visualize
...
def train(cfg):
    ...
    py_reader, avg_loss, lr, pred, [[seg1, seg2], [edge]], [grts, label_edge], masks = build_model(
    ...
    if args.debug:
        ...
        fetch_list.extend([pred.name, seg1.name, seg2.name, edge.name, grts.name, label_edge.name, masks.name])
        ...
        while True:
            try:
                if args.debug:
                    ...
                    loss, lr, pred, seg1, seg2, edge, grts, label_edge, masks = exe.run(
