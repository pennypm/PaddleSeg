...
from data_ace2p import get_affine_points 
...
def visualize(cfg,
    ...
    color_map = np.array(flatten(get_color_map(cfg.DATASET.NUM_CLASSES))) #256)
    ...
    for imgs, grts, img_names, valid_shapes, org_shapes in test_reader:
        ...
        def ACE2P_post(logit, org_shape):
            #logit = np.transpose(logit, [1, 2, 0])
            points, points_trans = get_affine_points(org_shape, (473, 473))
            M = cv2.getAffineTransform(np.float32(points_trans), np.float32(points))
            out = cv2.warpAffine(logit, M, tuple(org_shape[::-1]), flags=cv2.INTER_LINEAR)
            out = np.argmax(out, axis=2).astype(np.uint8)
            return out
        
        pred = np.transpose(logit, [0, 2, 3, 1])
        ...
        for i in range(num_imgs):
            ...
            res_map = ACE2P_post(res_map, org_shape)
            ...
            output_im = PILImage.fromarray(np.asarray(res_map, dtype=np.uint8))
            output_im.putpalette(color_map)
            output_im.save(vis_fn)
